...
func NewCustomResourceGenerator(crd unstructured.Unstructured) (CustomResourceGenerator, error) {
	...
	if foundValidation { // If present, use the top-level schema to validate all versions
		...
		} else if versionInfos, foundVersionInfos, _ := unstruct.NestedMapSlice(crd.Object, "spec", "versions"); foundVersionInfos {
		...
		versionInfos, foundVersionInfos, _ := unstruct.NestedMapSlice(crd.Object, "spec", "versions")
